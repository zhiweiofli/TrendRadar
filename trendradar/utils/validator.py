...
class ConfigValidator:
    ...
    def _validate_platforms(self, config: Dict) -> None:
        ...
        if not isinstance(platforms, list):
            raise ConfigError(
                "平台配置必须是列表类型",
                "请检查 config.yaml 中 platforms 的格式"
            )
    ...
    def _validate_report_mode(self, config: Dict) -> None:
        ...
        if not report_mode:
            raise ConfigError(
                "报告模式 (REPORT_MODE) 未配置",
                "请在 config.yaml 中配置 report.mode"
            )
...
def validate_frequency_words_file(file_path: str) -> bool:
    ...
    try:
        ...
        if not lines:
            raise ConfigError(
                "关键词文件为空",
                "请在 config/frequency_words.txt 中添加至少一个关键词"
            )
