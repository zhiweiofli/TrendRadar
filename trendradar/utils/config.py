...
def _build_config_dict(config_data: Dict) -> Dict:
    ...
    config = {
        ...
        "ENABLE_ASYNC": config_data["crawler"].get("enable_async", True),  # 新增异步开关
        ...
        # 静默推送配置
        "SILENT_PUSH": {
            "ENABLED": config_data["notification"].get("silent_push", {}).get("enabled", False),
            "TIME_RANGE": {
                "START": config_data["notification"].get("silent_push", {})
                        .get("time_range", {}).get("start", "08:00"),
                "END": config_data["notification"].get("silent_push", {})
                      .get("time_range", {}).get("end", "22:00"),
            },
            "ONCE_PER_DAY": config_data["notification"].get("silent_push", {})
                           .get("once_per_day", True),
            "RECORD_RETENTION_DAYS": config_data["notification"].get("silent_push", {})
    ...
    config["FEISHU_WEBHOOK_URL"] = os.environ.get("FEISHU_WEBHOOK_URL", "").strip() \
                                   or webhooks.get("feishu_url", "")
    config["DINGTALK_WEBHOOK_URL"] = os.environ.get("DINGTALK_WEBHOOK_URL", "").strip() \
                                     or webhooks.get("dingtalk_url", "")
    config["WEWORK_WEBHOOK_URL"] = os.environ.get("WEWORK_WEBHOOK_URL", "").strip() \
                                   or webhooks.get("wework_url", "")
    config["TELEGRAM_BOT_TOKEN"] = os.environ.get("TELEGRAM_BOT_TOKEN", "").strip() \
                                   or webhooks.get("telegram_bot_token", "")
    config["TELEGRAM_CHAT_ID"] = os.environ.get("TELEGRAM_CHAT_ID", "").strip() \
