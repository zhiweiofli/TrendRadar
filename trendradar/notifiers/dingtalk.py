...
class DingTalkNotifier(BaseNotifier):
    ...
    def send(
        ...
        try:
            ...
            logger.info(f"钉钉消息分为 {len(batches)} 批次发送 [{report_type}]")
            
            # 逐批发送
            for i, batch_content in enumerate(batches, 1):
                batch_size = len(batch_content.encode("utf-8"))
                logger.info(f"发送钉钉第 {i}/{len(batches)} 批次，大小：{batch_size} 字节 [{report_type}]")
                ...
                if response.status_code == 200:
                    ...
                    if result.get("errcode") == 0:
                        logger.info(f"钉钉第 {i}/{len(batches)} 批次发送成功 [{report_type}]")
                        # 批次间间隔
                        if i < len(batches):
                            time.sleep(self.batch_interval)
                    else:
                        logger.error(f"钉钉第 {i}/{len(batches)} 批次发送失败 [{report_type}]，错误：{result.get('errmsg')}")
                        return False
                else:
                    logger.error(f"钉钉第 {i}/{len(batches)} 批次发送失败 [{report_type}]，状态码：{response.status_code}")
                    return False
            
            logger.info(f"钉钉所有 {len(batches)} 批次发送完成 [{report_type}]")
            return True
        
        except Exception as e:
            logger.error(f"钉钉通知发送出错 [{report_type}]：{e}", exc_info=True)
            return False
