...
class TestDataFetcher:
    ...
    @patch('requests.get')
    def test_fetch_platform_sync_success(self, mock_get, sample_config, sample_platform, sample_api_response):
        """测试同步抓取成功"""
        # Mock 响应
        mock_response = Mock()
    ...
    @patch('requests.get')
    def test_fetch_platform_sync_failure(self, mock_get, sample_config, sample_platform):
        """测试同步抓取失败"""
    ...
    @patch('requests.get')
    def test_fetch_platform_sync_invalid_status(self, mock_get, sample_config, sample_platform):
        """测试同步抓取 - 无效状态"""
        mock_response = Mock()
    ...
    @pytest.mark.asyncio
    async def test_fetch_platform_async_success(self, sample_config, sample_platform, sample_api_response):
        """测试异步抓取成功"""
        # Mock aiohttp 响应
    ...
    async def test_fetch_all_async(self, mock_session_class, sample_config, sample_api_response):
        """测试异步抓取所有平台"""
        # Mock aiohttp 会话和响应
        mock_response = AsyncMock()
        mock_response.json = AsyncMock(return_value=sample_api_response)
